...
RUN echo @testing http://nl.alpinelinux.org/alpine/edge/testing >> /etc/apk/repositories && \
    ...
    docker-php-ext-install iconv pdo_mysql pdo_sqlite mysqli gd exif xsl json soap dom zip && \
...
# Setup PHP-FPM Config directory
RUN mkdir -p /usr/local/etc/php-fpm.d
RUN cp /usr/local/etc/php-fpm.conf.default $fpm_conf

# tweak php-fpm config
RUN echo "cgi.fix_pathinfo=0" > ${php_vars} &&\
    echo "upload_max_filesize = 150M"  >> ${php_vars} &&\
    echo "post_max_size = 152M"  >> ${php_vars} &&\
    ...
    sed -i \
        ...
        ${fpm_conf} && \
    CMS_ENV=$CMS_ENV && echo "env[CMS_ENV] = ${CMS_ENV}" >> ${fpm_conf}
